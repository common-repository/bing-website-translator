...
class PrisnaBWTCommon {
	...
	public static function getAdminPluginUrl() {
		...
		return admin_url('plugins.php?page=' . PrisnaBWTConfig::getAdminHandle());
	...
	public static function getVariableSet($_var_base_name, $_variants=array(), $_discard_empty=true, $_method='POST', $_escape_html=true) {
		...
		for ($i=0; $i<count($_variants); $i++) {
			...
			for ($j=0; $j<count($partial); $j++) {
				...
				if (!PrisnaBWTValidator::isEmpty($partial[$j]) || !$_discard_empty) {
	...
	public static function chain($primary_field, $parent_field, $sort_field, $rows, $root_id=0, $maxlevel=250000) {
		...
		$chain = new PrisnaBWTChain($primary_field, $parent_field, $sort_field, $rows, $root_id, $maxlevel);
		return $chain->get();
	...
	*/
	...
	public static function renderObject($_object, $_options=null, $_htmlencode=false) {
		...
		if ($_options['type'] == 'file')
			$template = PRISNA_BWT__TEMPLATES . $_options['content'];
	...
	protected static function displayHideBlock($_name, $_html, $_state) {
		...
		if ($_state) {
			...
			while ((!PrisnaBWTValidator::isEmpty($occurrence_ini)) && (PrisnaBWTValidator::isInteger($occurrence_ini)) && (!PrisnaBWTValidator::isEmpty($occurrence_end)) && (PrisnaBWTValidator::isInteger($occurrence_end))) {
				$positions[] = array (
					$occurrence_ini,
					$occurrence_end
				);
...
class PrisnaBWTUI extends WP_Widget {
	
	public function __construct() {
		
		parent::__construct(PrisnaBWTConfig::getWidgetName(true), PrisnaBWTConfig::getWidgetName(), array(
			'description' => sprintf(__('Add the %s.', 'prisna-bwt'), PrisnaBWTConfig::getName(false, true))
		));
	...
	public static function _initialize_widget() {
		...
		register_widget('PrisnaBWTUI');

	}

}

add_action('widgets_init', array('PrisnaBWTUI', '_initialize_widget'));

class PrisnaBWTValidator {
...
class PrisnaBWTFastJSON {
	 ...
	 *
	 *	PrisnaBWTFastJSON::convert(params:* [, result:Instance]):*
	 *
	 ...
	 *
	 * @note	please read Special PrisnaBWTFastJSON::convert method Informations
	 */
		...
		switch(gettype($params)){
			case	'array':
					...
					foreach($params as $key => $value) {
						if(($value = PrisnaBWTFastJSON::encode($value)) !== '')
							array_push($tmp, PrisnaBWTFastJSON::encode(strval($key)).':'.$value);
					};
					$result = '{'.implode(',', $tmp).'}';
					break;
			case	'boolean':
			...
			case	'object':
					...
					if(is_object($result)) {
						...
						foreach($result as $key => $value) {
							if(($value = PrisnaBWTFastJSON::encode($value)) !== '')
								array_push($tmp, PrisnaBWTFastJSON::encode($key).':'.$value);
						};
						$result = '{'.implode(',', $tmp).'}';
					}
					break;
	 ...
	 * public method
	 *
	 *	PrisnaBWTFastJSON::decode(params:String[, useStdClass:Boolean]):*
	 *
	 ...
	 *
	 * @example
	 *		PrisnaBWTFastJSON::decode('{"param":"value"}'); // associative array
	 *		PrisnaBWTFastJSON::decode('{"param":"value"}', true); // stdClass
	 *		PrisnaBWTFastJSON::decode('["one",two,true,false,null,{},[1,2]]'); // array
	 */
	public static function decode($encode, $stdClass = false){
		...
		if($slen !== null) {
			...
			set_error_handler(array('PrisnaBWTFastJSON', '__exit'));
			$result = PrisnaBWTFastJSON::__decode($encode, $pos, $slen, $stdClass);
	 ...
	 * public method
	 *
	 *	PrisnaBWTFastJSON::encode(params:*):String
	 *
	 ...
	 *
	 * @example
	 *		PrisnaBWTFastJSON::encode(array(1,"two")); // '[1,"two"]'
	 *
	 *		$obj = new MyClass();
	 *		obj->param = "value";
	 *		obj->param2 = "value2";
	 *		PrisnaBWTFastJSON::encode(obj); // '{"param":"value","param2":"value2"}'
	 */
			...
			case	'array':
					...
					if (count(array_keys($decode)) == 2 && array_key_exists('value', $decode) && array_key_exists('type', $decode) && $decode['type'] == 'literal')
						...
						if(!count($decode) || array_keys($decode) === range(0, count($decode) - 1)) {
							...
							$result = PrisnaBWTFastJSON::convert($decode);
							
					}
					break;
			case	'string':
					$replacement = PrisnaBWTFastJSON::__getStaticReplacement();
	...
	protected static function __decode(&$encode, &$pos, &$slen, &$stdClass){
		switch($encode{$pos}) {
			...
			case '[':
				...
				++$pos;
				while($encode{$pos} !== ']') {
					array_push($result, PrisnaBWTFastJSON::__decode($encode, $pos, $slen, $stdClass));
					if($encode{$pos} === ',')
						++$pos;
				}
				++$pos;
			...
			case '{':
				...
				++$pos;
				while($encode{$pos} !== '}') {
					$tmp = PrisnaBWTFastJSON::__decodeString($encode, $pos);
					++$pos;
					if($stdClass)
						$result->$tmp = PrisnaBWTFastJSON::__decode($encode, $pos, $slen, $stdClass);
					else
						$result[$tmp] = PrisnaBWTFastJSON::__decode($encode, $pos, $slen, $stdClass);
					if($encode{$pos} === ',')
						++$pos;
			...
			case '"':
				switch($encode{++$pos}) {
					case '"':
						...
						break;
					default:
						$result = PrisnaBWTFastJSON::__decodeString($encode, $pos);
						break;
				}
				++$pos;
				break;
	...
	protected static function __decodeString(&$encode, &$pos) {
		$replacement = PrisnaBWTFastJSON::__getStaticReplacement();
		$endString = PrisnaBWTFastJSON::__endString($encode, $pos, $pos);
	...
	protected static function __endString(&$encode, $position, &$pos) {
		...
		}while($position !== false && PrisnaBWTFastJSON::__slashedChar($encode, $position - 1));
		if($position === false)
	...
	protected static function __exit($str, $a, $b) {
		exit($a.'FATAL: PrisnaBWTFastJSON decode method failure [malicious or incorrect JSON string]');
	}
